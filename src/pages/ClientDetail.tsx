...
const sidebarTabs = ['Overview', 'Documents', 'Credential Vault', 'Tally Sync', 'Filings', 'Tasks', 'Billing', 'Activity'];
  ...
  return (
    <div className="detail-layout">
      <nav className="sidebar" aria-label="Client sections">
        {sidebarTabs.map((tab) => (
          <span key={tab} className={`sidebar__item${tab === 'Overview' ? ' sidebar__item--active' : ''}`}>
            {tab}
          </span>
        ))}
      </nav>
      ...
      <main className="detail-main">
        <div className="detail-header">
          ...
          {loading || !data ? (
            ...
            <>
              <h1 className="detail-header__name">{data.client.legal_name}</h1>
              <span className={`client-status client-status--${data.client.status}`}>{data.client.status}</span>
            </>
          )}
        </div>
